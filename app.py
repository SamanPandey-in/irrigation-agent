...
@app.get("/health")
async def health():
    return {"status": "healthy", "version": "1.0.0", "time": time.time()}

@app.get("/metadata")
async def metadata():
    return {
        "name": "precision-irrigation-agent",
        "description": "Precision irrigation RL environment for Punjab rice farming. Controls irrigation across a 90-day Kharif rice season.",
        "version": "1.0.0",
    }
...
async def schema():
    return {
        ...
        "action": {
            "type": "discrete",
            "n": 5,
            "labels": {"0": "No Water", "1": "Low (1kL)", "2": "Medium (3kL)", "3": "High (5kL)", "4": "Drain"},
        },
        "state": {
            "type": "dict",
            "description": "Full environment state including soil moisture, crop growth, and weather forecast.",
        },
    }
